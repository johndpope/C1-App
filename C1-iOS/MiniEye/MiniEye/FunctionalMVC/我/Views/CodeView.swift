...
extension CodeView {
    ...
    fileprivate func creatLineView() {
        for num in 0 ..< Int(Base.codeNum) {
            ...
            let lineView = UIView(frame: CGRect(x: x+x1, y: height -  Length.onceHeight.rawValue, width: 40, height: 0.5))
...
extension CodeView : UITextFieldDelegate {
    ...
    func textFieldDidBeginEditing(_ textField: UITextField) {
        ...
        if text.count < Int(Base.codeNum) {
            layerArr[text.count].isHidden = false
        }
    }
